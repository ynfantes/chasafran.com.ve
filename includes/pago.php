...
class pago extends db implements crud {
    ...
    public function listarPagosPendientes(){
        $query = "select distinct p.* "
                . "from pagos p join pago_detalle d on p.id = d.id_pago where estatus = 'p' ";
    ...
    public function registrarPago($data) {
        ...
        try {
            ...
            if ($res['suceed'] && !count($res['data']) > 0 ) {
                ...
                if ($res['data'][0]['estatus']=='a') {
                    $resultado['mensaje'].= "\nEl pago ya fue rechazado. Si considera que es un error nuestro, escríbanos a chasafran1@gmail.com";
                }
            }
        } catch (Exception $exc) {
            $resultado['suceed'] = false;
    ...
    public static function listarPagosProcesadosWeb() {
        $consulta = "select d.id_factura,d.id_inmueble,d.id_apto from pagos p join pago_detalle d on p.id = d.id_pago "
                ...
                . "(select id from inmueble) and d.periodo >= '2017-09-01 00:00:00' order by d.id_inmueble, d.id_apto, d.periodo DESC";
